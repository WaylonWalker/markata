...
def make_index(tmp_path_factory):
...
def test_loaded(make_index) -> None:
...
def test_markout_exists(test_run):
    markout = test_run.getbasetemp() / "markout"
    assert markout.exists()


def test_index_exists(test_run):
    markout = test_run.getbasetemp() / "markout"
    index = markout / "index.html"
    assert index.exists()


def test_rss_exists(test_run):
    markout = test_run.getbasetemp() / "markout"
    rss = markout / "rss.xml"
    assert rss.exists()
